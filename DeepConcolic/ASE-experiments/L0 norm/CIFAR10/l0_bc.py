"""
Boundary Cover of CIFAR10 based on L0 Norm
...
def boundary_cover(model, activations, nc_layer, nc_index, image, bc_udi, bc_v, layer_functions):
    idx_min = 0
    idx_max = 10
    ...
    if is_conv_layer(model.layers[l]):
        ...
        tic = time.time()
        sorted_features = influential_pixel_manipulation(image, model, label, pixel_num, ii, jj, kk, l, bc_udi, bc_v,
                                                         layer_functions)
        (adv_images, adv_labels, idx_first, success_flag) = accumulated_pixel_manipulation(image, model,
                                                                                           sorted_features, mani_range,
                                                                                           label, ii, jj, kk, l, bc_udi,
        ...
        tic = time.time()
        sorted_features = influential_pixel_manipulation_dense(image, model, label, pixel_num, hh, l, bc_udi, bc_v,
