...
from utils_guap import *



def run_g(dataSet, lr, batch_size, epochs, l2reg, beta1, tau, eps, model_name, manualSeed, gpuid, cuda, resume, outdir, limited):
    ...
    optimizer = torch.optim.Adam(netAttacker.parameters(), lr=lr, betas=(beta1, 0.999), weight_decay=l2reg)
    ...
    for epoch in range(epochs):
        ...
        for i, (X, y) in enumerate(train_loader):
            ...
            if limited =='true' and i == (len(train_loader)//10):
                break
        ...
        list_Time.append(train_time - start_time)
        list_Tr_loss.append(train_loss / train_n)
        list_Tr_acc.append(train_ori_acc/train_n)
        list_Tr_stASR.append(train_st_rate/train_n)
        list_Tr_noiseASR.append(train_noise_rate/train_n)
        list_Tr_GUAP_ASR.append(train_attack_rate/train_n)
    ...
    Time = test_time - start_time
    Test_loss = test_adv_loss/test_n
    Ori_Acc = test_ori_acc/test_n
    Perb_Acc = test_adv_acc/test_n
    Te_stASR = test_st_rate/test_n
    Te_noiseASR = test_noise_rate/test_n
    Te_GUAP_ASR = test_attack_rate/test_n
    ...
    return list_Epoch, list_Time, list_Tr_loss, list_Tr_acc, list_Tr_stASR, list_Tr_noiseASR, list_Tr_GUAP_ASR, Time, Test_loss, Ori_Acc, Perb_Acc, Te_stASR, Te_noiseASR, Te_GUAP_ASR
